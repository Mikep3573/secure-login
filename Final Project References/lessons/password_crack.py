...
def hash_pw(plain_text, salt='') -> str:
    ...
    # ---MY CODE
    # NOTE: I removed the 'salt' parameter since we're making our random one here
    # Generate a pseudorandom salt
    salt = os.urandom(20)  # Create a pseudorandom set of 20 bytes
...
def authenticate(stored, plain_text, salt_length=None) -> bool:
    ...
    with open('./instance/static/output.txt', 'w') as output_file:
        output_file.write(stored + '\n' + this_hash + '\n' + stored_hash)
